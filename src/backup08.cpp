...
const double LIDAR_VIEW_ANGLE_H = 180.0 * DEG_TO_RAD; // 障害物を探索する水平画角
const double LIDAR_VIEW_ANGLE_V = 32.0 * DEG_TO_RAD; // 障害物を探索する垂直画角
const double LIDAR_RESOLUTION_H = 0.36 * DEG_TO_RAD; // LiDARの水平解像度(分解能)
const double LIDAR_RESOLUTION_V = 1.0 * DEG_TO_RAD; // LiDARの垂直解像度(分解能)
const double DETECT_HIGHT_MIN = 0.3; // 障害物の最小の高さ
const double DETECT_HIGHT_MAX = 1.5; // 障害物の最大の高さ

const double LIDAR_HEIGHT = 0.97; // LiDARの取り付け高さ
const double REFLECTIVITY_THRESH = 0.001; // 反射強度の閾値

const double MAX_RANGE = 20.0f;
const double CLUSTERING_DISTANCE = 0.5; // 同一物体と判断する距離
const double MIN_OBJECT_SIZE = 50; // 検出する物体の最小の画素数
const int FEATURE_HOG_BIN_NUM = 60; // HOGのビンの数
const int FEATURE_REF_BIN_NUM = 50; // 反射強度ヒストグラムのビンの数
const double MATCH_THRESHOLD = 0.7; // マッチ判定の閾値
const int OBSTACLE_BUFFER_SIZE = 2; // バッファとして確保する過去の障害物の数
...
void projectToImage(const vector<LidarData> &points,Mat &lidar_img)
{
  ...
  for(int i=0; i<sz;i++)
  {
    // if(points[i].z < DETECT_HIGHT_MIN || points[i].z > DETECT_HIGHT_MAX) continue; // 高さ制限
  ...
  // cout << "lidar_img_rfl.size().width : " << lidar_img_rfl.size().width << endl;
  // cout << "lidar_img_rfl.size().height : " << lidar_img_rfl.size().height << endl;
}
...
void extractRectangles(const Mat &clusters, int cluster_num, vector<Rect> &rects)
{
  ...
  for(int y=0; y<clusters.rows; y++)
  {
    for(int x=0; x<clusters.cols; x++)
    {
      ...
      if(ul[p].x > x) ul[p].x = x;
      if(ul[p].y > y) ul[p].y = y;
      if(br[p].x < x) ul[p].x = x;
      if(br[p].y < y) ul[p].y = y;
    }
  }
...
// void relativeToAbsolute(const Pose3D &pose, std::vector<Obstacle> &obstacles)
// {
//   int sz1 = obstacles.size();
//   for(int i=0; i<sz1; i++)
//   {
//     const int sz2 = obstacles[i].relative_points.size();
//     double s = sin(pose.yaw), c = cos(pose.yaw);
//     if(obstacles[i].absolute_points.size() != sz2) obstacles[i].absolute_points.resize(sz2);
//     for(int j=0;j<sz2; j++)
//     {
//       obstacles[i].absolute_points[j].x = c * obstacles[i].relative_points[j].x - s * obstacles[i].relative_points[j].y + pose.x;
//       obstacles[i].absolute_points[j].y = s * obstacles[i].relative_points[j].x + c * obstacles[i].relative_points[j].y + pose.y;
//     }
//   }
// }
...
void calculateHog(const Mat &cluster, int id, vector<float> &hog)
{
  ...
  //     if(amp < 0.1) continue; // 0.1°よりも小さかった場合
  //     float rad = atan2(dy, dx) * 180.0f / M_PI; // atan2(dy, dx)ではなく？
...
void extractObstacleFeatures(const TrackableObstacle &obstacle, vector<ObstacleFeature> &features)
{
  ...
  for(int i = 0; i<obstacle.size; i++)
  {
    ...
    cout << "i : " << i << endl;
    cout << "r.x : " << obstacle.rectangles[i].x <<endl;
    cout << "r.y : " << obstacle.rectangles[i].y <<endl;
    cout << "r.width : " << obstacle.rectangles[i].width <<endl;
    cout << "r.height : " << obstacle.rectangles[i].height <<endl;
    // calculateHog(obstacle.clusters(r), i, features[i].hog); //エラーの原因
...
void detect(const vector<LidarData> &points, TrackableObstacle &obstacle)
{
  ...
  max_id = integrateLabel(id_pairs, obstacle.clusters, max_id);
  obstacle.size = relabel(obstacle.clusters, max_id);
  ...
  // relativeToAbsolute(pose, obstacle,obstacle.features);
  // extractObstacleFeatures(obstacle, obstacle.features); // エラーの原因
  // obstacles_buffer.push_back(obstacle);
  // points_buffer.push_back(points);
}

void drawObjects(const TrackableObstacle &obstacle, Mat &img)
{
  auto remap = [](float val, float from_low, float from_high, float to_low, float to_high)
  {
    return (val - from_low) * (to_high - to_low) / (from_high - from_low) + to_low;
  ...
  for(int y=0;y<img.rows; y++)
  {
    ...
    {
      float d = lidar_img.at<Vec2f>(y,x)[1];
      if(d>MAX_RANGE || d<= .0f) continue;
      int v = (int)remap(d, 0.0f, (float)MAX_RANGE, 30.0f, 255.0f);
...
int main(int argc,char **argv){
  ...
  while (true)
  {
    ...
    //////////////////////////////////////////////////////////////////////
    drawObjects(obstacle, lidar_range_img);
    //////////////////////////////////////////////////////////////////////
