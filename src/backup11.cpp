...
void rectangleReflect(const Mat &lidar_reflect_img, const Mat &camera_img, 
                      ...
                      vector<int> &pts1_x_blob, vector<int> &pts1_y_blob, vector<int> &pts2_x_blob, vector<int> &pts2_y_blob)
{
  ...
  for(int label = 0; label < object_reflect_id; label++)
  {
    ...
    cv::rectangle(camera_img, rect_sign, cv::Scalar(255,255,0), 2);
    ...
    int pt1_x_blob, pt1_y_blob, pt2_x_blob, pt2_y_blob;
    if(camera_left - 3*camera_width < 0){
      pt1_x_blob = 0;
    }else{
      pt1_x_blob = camera_left - 3*camera_width;
    }
    if(camera_top - 0.5*camera_height < 0){
      pt1_y_blob = 0;
    }else{
      pt1_y_blob = camera_top - 0.5*camera_height;
    }
    if(camera_left + camera_width + 3*camera_width > camera_img.size().width){
      pt2_x_blob = camera_img.size().width;
    }else{
      pt2_x_blob = camera_left + camera_width + 3*camera_width;
    }
    if(camera_top + camera_height + 0.5*camera_height > camera_img.size().height * 0.5){
      pt2_y_blob = camera_img.size().height * 0.5;
    }else{
      pt2_y_blob = camera_top + camera_height + 0.5*camera_height;
    }
    pts1_x_blob.push_back(pt1_x_blob);
    pts1_y_blob.push_back(pt1_y_blob);
    pts2_x_blob.push_back(pt2_x_blob);
    pts2_y_blob.push_back(pt2_y_blob);
    pts1_x_blob[label] = pt1_x_blob;
    pts1_y_blob[label] = pt1_y_blob;
    pts2_x_blob[label] = pt2_x_blob;
    pts2_y_blob[label] = pt2_y_blob;
    cv::Rect rect_blob(Point(pts1_x_blob[label], pts1_y_blob[label]), Point(pts2_x_blob[label], pts2_y_blob[label]));
    cv::rectangle(camera_img, rect_blob, Scalar(0, 255, 0), 2);
  }
}
...
int main(int argc,char **argv){
  ...
  while (true)
  {
    ...
    vector<int> pts1_x_blob, pts1_y_blob, pts2_x_blob, pts2_y_blob;
    ...
    rectangleReflect(lidar_reflect_img, camera_img,
                      ...
                      pts1_x_blob, pts1_y_blob, pts2_x_blob, pts2_y_blob);
