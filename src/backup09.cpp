...
const double LIDAR_VIEW_ANGLE_H = 140.0 * DEG_TO_RAD; // 障害物を探索する水平画角
...
const double DETECT_HEIGHT_MIN = 0.3; // 障害物の高さ(最小)
const double DETECT_HEIGHT_MAX = 1.5; // 障害物の高さ(最大)
const double LIDAR_HEIGHT = 0.97; // LiDARの取り付け高さ
const double REFLECT_THRESH = 0.9f; // 反射強度の閾値
const double MAX_RANGE = 15.0f; // 投影する距離の最大値
const double MAX_REFLECT = 1.0f; // 投影する反射強度の最大値
...
void drawObjectsReflect(const TrackableObstacle &obstacle, Mat &img)
{
  ...
  for(int y=0;y<img.rows; y++)
  {
    for(int x=0; x<img.cols; x++)
    {
      ...
      if(reflect>MAX_REFLECT || reflect<= .0f) continue;
      if(range>MAX_RANGE || range<= .0f) continue;
      if(reflect<REFLECT_THRESH) continue;
