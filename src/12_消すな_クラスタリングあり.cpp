                      ...
                      vector<int> &pts1_x_region, vector<int> &pts1_y_region, vector<int> &pts2_x_region, vector<int> &pts2_y_region)
{
  ...
  for(int label = 0; label < object_reflect_id; label++)
  {
    ...
    pts1_x_region.push_back(pt1_x_blob);
    pts1_y_region.push_back(pt1_y_blob);
    pts2_x_region.push_back(pt2_x_blob);
    pts2_y_region.push_back(pt2_y_blob);
    pts1_x_region[label] = pt1_x_blob;
    pts1_y_region[label] = pt1_y_blob;
    pts2_x_region[label] = pt2_x_blob;
    pts2_y_region[label] = pt2_y_blob;
    cv::Rect rect_blob(Point(pts1_x_region[label], pts1_y_region[label]), Point(pts2_x_region[label], pts2_y_region[label]));
...
int main(int argc,char **argv){
  ...
  while (true)
  {
    ...
    /*** 変換処理 ***/
    remap(camera_img, dst, map_x, map_y, INTER_LINEAR);
    cv::imshow("dst",dst);
    /* 点群の読み込み */
